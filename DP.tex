...
Návrh této aplikace obsahuje autentizaci pomoví JSON tokenu zvanou JWT\footnote{JWT-JSON web token}\cite{JWT}
...
JWT token jsou tři base64 textové řetězce za sebou odělené tečkami.\\
Ukázka JWT:
\begin{codeframe}
  \begin{verbatim}
<header>.<payload>.<hash>
...
\b{payload:} může býť jakýkoliv validní JSON objekt, ale vyžaduje některé standartizované klíče.
\begin{codeframe}
  \begin{verbatim}
{ "user": "Franta", "exp": 123456789}
    \end{verbatim}
\end{codeframe}
user je klíč definovaný a~exp je standartní UNIX time s~časem do kdy token platí. To že payload může být libovolný je skvélá věc, protože~můžeme přenášet jakékoliv informace o~uživateli. Důležité je vědět, že~payload není šifrovaný, jedná se pouze o~base64 zakódování, a nemůže být tedy použit k~přenášení citlivých informací.\\
