...
Diplomová práce je zaměřena na proces, kdy už od počátku tvorby může vydavatel zasahovat do díla autora, protože autor začíná dílo tvořit přímo v~systému vydavatele. Výhodu je, že~je~práce autorovi dostupná všude kde se dostane k~počítači a~může být od počátku tvořena v~šabloně vydavatele a~dodržovat tak vysoký standard kvality. Samozřejmě je možné dílo nahrát již hotové.\\
...
\end{figure}
Coffee-script je malý a~jednoduchý jazyk, který je traspiluje \footnote{transpilovat znamená převádět kód z jednoho programovacího jazyka na druhý} do javascriptu. Přispívá k~jednoduchosti kódu a~urychluje jeho psaní, protože vypouští závorky, přidává některou funkcionalitu ES6 \footnote{ES6 neboli ECMAScript6 je nástupce současného standardu ECMAScript5 používaného většinou prohlížečů, přináší podporu pro třídy,moduly, iterátor, yield, a další.} výrazně zjednodušuje cykly a~mnoho dalšího,…\\
...
Ale protože bude ještě chvíli trvat než bude ES6 hotový a~podporovaný napříč prohlížeči, byly vytvořeny různé implementace některých jeho nových funkcí pomocí ECMAstript 5.1, jednou z~funkcí, které byly implementovány jsou \b{Moduly}.\\
Těchto implementací existuje hned několik. NodeJS používá CommonJS moduly na straně serveru a~pro prohlížeč jsou k~dispozici AMD\footnote{AMD - Asynchronous Module Definition} moduly, které používá RequireJS. Další možností jsou však BrowserifyJS moduly, které mají stejnou syntaxi jako CommonJS moduly využívané v~NodeJS a~umožňují tak importovat CommonJS moduly přímo do prohlížeče.\cite{browserify}
     \begin{codeframe} 
      \begin{Verbatim}{frame=single}
# obsah.coffee
module.exports = (x, y) ->
...
\end{Verbatim} 
    \end{codeframe}
V souboru obsah.coffee exportujeme funkci, která příjímá parametry x,y a~vrací obsah obdélníku. V~souboru app.coffee voláme soubor obsah.coffee, který~je ve společné složce, to je však funkce takže jí můžeme předat vstupní parametry 2,2. v~proměnné obsah pak bude uložen výsledek 4.\\
Moduly umožňují asynchronní načítání svých závislostí, které se načtou vždy pouze jednou, vytváří hierarchii aplikace a~strom závislostí jednotlivých komponent. Také umožňuje snadnější rozdělení aplikace na jednotlivé soubory.\cite{browserify}\cite{browserify2}
...
Podobně jako kombinace Linux+Apache+MySQL+PHP získala svoji široce známou zkratku \b{LAMP}, je pro označení často užívané kombinace nástrojů MongoDB + ExpressJS + AngularJS + NodeJS využívána zkratka MEAN. Pro tuto kombinaci nástrojů byl od začátky designován celý projekt.\cite{mean}\\
...
NPM také obsahuje interaktivní shell pro vytváření nových projektů, který vytvoří package.json za uživatele. Nově instalované moduly jsou při přidání přepínače \it{--save} nebo \it{--save-dev} přidávány také automaticky.\\
...
ExpressJS je webový framework inspirovaný webovým frameworkem Synatra napsaným v ruby. Jedná se o~malý flexibilní a~robustní framework pro pro psaní single-page, multi-page a hybridních stránek postavený na NodeJS.\cite{expressjs}
\begin{figure}[h]
\begin{center}$
\begin{array}{c}
...
\subsubsection{Pack}
Pack je funkcionalita, která umožňuje sdružování množství serverů do jedné logické jednotky. Takový pack může sdružovat několik serverů, které~mohou mít vlastní pluginy.\cite{hapiJSDOC}
     \begin{codeframe} 
      \begin{Verbatim}{frame=single}
Hapi = require("hapi")
pack = new Hapi.Pack()
...
\end{figure}
MongoDB je dokumentová NoSQL databáze s~dokumenty ve formátu json a~dynamickým schématem. Počátek jejího vývoje spadá do roku 2007 a~je vyvíjena společností 10gen, momentálně se nachází ve verzi 2.6.\\
...
# se místo reference objeví objekt konkrétní khiny.
\end{Verbatim} 
    \end{codeframe}
...
\end{figure}
LESS je pre-processorem CSS stylů a~jeho účelem je rozšířit funkcionalitu CSS jazyka. CSS je velmi jednoduché a~nenabízí dynamičnost, která je dnes velmi potřebná a~je často vyžadována projektem samotným. Právě k tomuto účelu se používá pre-procesor. Zejména jsou v~dnešní době používány následující pre-procesory \it{LESS, SASS a Stylus}. Ve své podstatě jsou si podobné a~nabízí i~podobnou funkcionalitu. Všechny mají stejné vnořené selectory, svůj systém proměnných, cykly, mixin, funkce a~mnoho dalších nástrojů usnadňujících práci developerům.\cite{lesscss}\\
...
\begin{center}$
\begin{array}{c}
\includegraphics[scale=0.5]{./img/twitter-bootstrap.jpg}
\end{array}$
\end{center}
\caption[Logo Twitter Bootstrap]{Logo Twitter Bootstrap}
\end{figure}

Twitter Bootsrap je jeden z~prvních a~určitě nejpoužívanější frontend framework\footnote{\b{Frontend frameworky} v~určitých segmentech zpřístupňují nástroje webového frontendu masám podobně, jako to udělalo jQuery s~JavaScriptem\cite[bootstrap], vlastně spojují hodně vlastností do jedné, organizace kódu, CSS preprocesory,JS knihovny,... z~toho všeho vytváří sadu CSS a~JS knihoven, které~mají standardizované API a~zaručují funkčnost na podporovaných prohlížečích} současné doby.\\
Twitter boostrap disponuje responzivním grid systémem, vlastní sadou CSS a~JS komponent, modulární strukturou, standardizovaným API, výbornou dokumentací a~mnohými dalšími funkcemi.\\
Co je důležité Twitter Bootstrap podporuje od verze 3 CSS preprocesory a~je dostupný pro LESS i~SASS. Což umožňuje ve stylech naší aplikace importovat části Bootsrapu.
\begin{codeframe}
  \begin{verbatim}
...
@import "vendor/bootstrap/less/grid";
  \end{verbatim}
\end{codeframe}
dostaneme tak přímo do našich LESS stylů možnost využívat Bootsrap mixiny a~měnit např. proměnné a~přímo tak ovlivňovat výslednou podobu Bootsrap knihovny\cite{bootstrap2}.\\

Ukázka použití Twitter boostrap v~html kódu:\cite{bootstrap3}
\begin{codeframe}
  \begin{verbatim}
<div class="row">
  ...
  \end{verbatim}
\end{codeframe}
Popis dalších funkcí a~vlastností Twitter Bootstrap je možné nalézt na oficiálním webu \href{http://getbootstrap.com/css/}{http://getbootstrap.com/css/}.

\subsection{LessHat}
\begin{figure}[h]
\begin{center}$
\begin{array}{c}
\includegraphics[scale=0.5]{./img/lesshat.png}
\end{array}$
\end{center}
\caption[Logo LessHat]{Logo LessHat}
\end{figure}

Twitter Bootstrap má vlastní sadu mixinů pro less, která je výborná většinou naprosto dostačující. LessHat je specializovaná knihovna mixinů, která obsahuje všechno na co si každý kodér vzpomene. Konkrétně obsahuje 87 mixinů a~starosti o~vendor prefixy a~nekompatibilitu mezi prohlížeči může kodér částečně strčit za hlavu.\cite{lesshat}\cite{lesshat2}\\

příklad výstupu mixinu pro gradient:
\begin{codeframe}
  \begin{verbatim}
//LESS
...
\end{figure}
Gulp je task runner stejně jako známější Grunt, který je ve světě NodeJS aplikací zřejmě nejpoužívanějším task runnerem. Grunt a Gulp dělají ve~své podstatě to samé, ale grunt má několik limitů, které překonává právě Gulp. Především se jedná o~návaznost úkolů. Grunt pracuje se soubory tak, že spustíme jeden taks např. překlad coffeescriptu, přeložené soubory uložíme, spustíme sloučení (concat) souborů, uložíme jinam spustíme uglifikaci apd,… Gulp používá datové streamy a~roury (pipes), kdy předává výstup jednoho úkolu druhému a~vše je tam v~jenom tasku. Konfigurace je tak mnohem jednodušší a~všechny úkoly probíhají asynchronně.\\
...
Prvním je pohled z~hlediska modulárnosti aplikace jako takové a~její škálovatelnosti a~druhým pohled z~hlediska uživatelského.\\
...
hlavní controllery a~vstupní body do jednotlivých aplikací
\begin{codeframe}
  \begin{verbatim}
/src/app
    \end{verbatim}
...
dočasné soubory, především \it{templates.js} a~\it{sprites.less}
\begin{codeframe}
  \begin{verbatim}
/tmp
  \end{verbatim}
...
favourite_books:
  [
    ...
    \end{verbatim}
\end{codeframe}
Další část dokumentu se zabývá především referencemi na knihy. Uživatel může mít velké množství objednávek, které samostatně obsahují všechny uživatelovi informace. Každý uživatel může tvořit také knihy takže mu náleží kolekce projektů, může si přát knihy, které mu mohou kupovat známí a~může mít označeny své oblíbené knihy. Velmi důležitou položkou jsou pak zaplacené knihy, podle tohoto pole systém pozná, které knihy může uživatel stáhnou ve formátu PDF nebo jestli si je koupil v~tiskovém vydání.
...
special:
  ...
  denied: [String]
    \end{verbatim}
\end{codeframe}
Následuje část zabývající se právy uživatele, uživatel může mít definované role a~zvláštní práva pro přístup k~funkcionalitám
...
last_login:
  ...
  \end{verbatim}
\end{codeframe}
Důležité je také uchovávat informace o~posledním přihlášení uživatele a~odkud byl uživatel přihlášen. Především kvůli bezpečnosti uživatele, kdy uživatel nebo i~systém automaticky může některé přihlášení vyhodnotit jako falešné, především na základě geolokačních dat IP adresy. Více údajů o~předchozích přihlášeních, lze pak vyčíst z~Auth logu \odkazNaKapitolu{database:log}.
...
created_at:
  ...
  \end{verbatim}
\end{codeframe}
Poslední částí, se bude opakovat téměř ve všech kolekcích takže nebude dále uvedena. Zabývá se především dalších popisků objektu a~časy vytvoření a poslední úpravy.
...
book.find(
  ...
  \end{verbatim}
\end{codeframe}
U náročnějšího systému by bylo pak zřejmě vhodné vytvořit pro autory ještě samostatnou kolekci, ale~údaje o~autorovi zanechávat také v~knihách a~kolekci autorů používat pouze pro vyhledání knih konkrétních autorů.
...
Objednávka je v SQL databázích většinou předmětem velkého množství relací na uživatele adresy, schvalování, dopravu apd,… Toto všechno NoSQL databáze řeší velmi elegantně v jediném dokumentu. Bez jakých kolik relací, které vy vyžadovali další odkazování. Navíc pokud je objednávka již jednou vytvořena neměl by její obsah být pozměněn referencemi.
\begin{codeframe}
  \begin{verbatim}
  user:
    name:
...
\begin{codeframe}
  ...
  \end{verbatim}
\end{codeframe}
Velmi důležitý je seznam zakoupených položek. Opět je část základních data zkopírována do dokumentu aby nedošlo ke~změnám v~objednávkách následkem změny reference.
...
\begin{codeframe}
  ...
  states:[
    ...
    \end{verbatim}
\end{codeframe}
Objednávka má samozřejmě svou historii a~to od chvíle jejího vytvoření až do chvíle jejího zrušení. Proto jsou v~této části definovány změny v~objednávce. Pokaždé když objednávka postoupí schvalovacím procesem, nebo změnou zapíše se zpráva co se stalo a~kdo akci vyvolal.
...
\begin{codeframe}
  ...
  delivery_method:
    ...
    branche:
      ...
      \end{verbatim}
\end{codeframe}
Poslední část se zabývá obchodem samotným. Uživatel má možnost zvolit vlastní identifikátor objednávky, metodu platby, kam bude objednávka zaslána, kam fakturována a~jestli byla objednávka z~e-shopu nebo z~pobočky.//

\subsubsection{Project}
Pro potřebu vytváření knih je zde kolekce Project, která zajišťuje vlastnosti potřebné pro vytvoření publikace. Kromě informací o~uživateli obsahuje
\begin{codeframe}
  \begin{verbatim}
  name:
    type: String
...
Knihkupectví většinou nemají pouze internetový obchod, ale také kamenné pobočky. Pro držení informací o~pobočkách slouží kolekce Branches.
\begin{codeframe}
  \begin{verbatim}
  name:
    type: String
  ...
  \end{verbatim}
\end{codeframe}
Kolekce poboček musí samozřejmě obsahovat informace o~pobočce samotné a také o jejích zaměstnancích, platech zaměstnanců apd,…
...
config.groups:[
  ...
  allowed:[String]
    ...
    \end{verbatim}
\end{codeframe}
Klíč \it{groups} zajišťuje nastavení skupin a~přístup k~jednotlivým akcím
...
config.actionsuserDefined:[String]
    \end{verbatim}
\end{codeframe}
Seznam akcí backendu, tyto klíč slouží pouze jako seznam funkcionalit ze~kterých je možné volit při přidělovaní práv. Vlastní seznam funkcionalit by mohli registrovat také jednotlivé pluginy při startu serveru.
...
Zabezpečení MongoDB databáze je velmi jednoduché a~lze jej zprovoznit pomocí MongoDB cli. Nejprve je třeba spustit program \it{"mongo"}, který je standardní součástí balíčku mongoDB pro většinu distribucí. Po spuštění se zobrazí interaktivní "mongo shell" ve, kterém je třeba nastavit administrátorské účty v~kolekci admin (bez nastavení účtů pro administrátory by se nedalo po zapnutí autentizace do databáze dostat).\cite{mongoAdmin}\\
...
db.createUser(
  ...
  \end{verbatim}
\end{codeframe}
Uživatelů je možné přidávat různá oprávnění např. readAnyDatabase,readWriteAnyDatabase,dbOwner,userAdmin,userAdminAnyDatabase,...\\
Jakmile je administrátor vytvořen je možné zapnout autentizaci. Zapnutí lze provézt v~souboru \it{/etc/mongo.conf} a~to přepsáním properky "auth" na "true". Pak je třeba restartovat mongoDB server a~je možné se přihlásit např. následujícím příkazem:
\begin{codeframe}
  \begin{verbatim}
mongo --port 27017 -u admin -p password --authenticationDatabase admin
  \end{verbatim}
\end{codeframe}
Nyní není možné se na MongoDB server dostat bez nutnosti přihlášení a~je možné vytvořit databázi pro projekt. K tomu slouží klíčové slovo \it{USE}, to říká kterou databázi budeme používat a~pokud neexistuje tak ji vytvoří.
\begin{codeframe}
  \begin{verbatim}
use mojeDatabaze
  \end{verbatim}
\end{codeframe}
Po založení databáze je již jen třeba vytvořit uživatele s~přístupem k~ní, což může být provedeno stejně jako při vytvoření systémového administrátora. Pokud chceme zabezpečit aplikaci ještě více, můžeme využívat dvou spojení do databáze, jednoho pro čtení a~jednoho pro zápis. Taková aplikace může mít např. i~dvě api jedno právě pro čtení a~druhé pro zápis.
...
\subsection{Struktura}
Struktura backendu odpovídá struktuře celého projektu a~je tvořena jednotlivými částmi popsanými v~následujícím diagramu. Na hoře je klient, což je angularJS frontend aplikace. Request od klienta putuje na server, kde jej zpracovává router frameworku HapiJS, který se stará o~validaci requestů (požadavku), autentizaci, přidělení funkcionality, validaci response (odpovědi) a~odeslání odpovědi zpět klientovy. Každý request tedy projde routerem, kde je mu přidělena funkce controlleru, provedena potřebná akce např. získání záznamů z~databáze pomocí Mongoose ODM apd,... a~odpověď je pak controllerem vrácena zpět klientovi.
\begin{figure}[h]
\begin{center}$
\begin{array}{c}
...
\subsection{API}
API aplikace je velmi jednoduché, další funkcionalita může být snadno přidána vytvořením modulu aplikace, který může mít vlastní router, view adp,... API je navíc od počátku verzované což umožňuje snažší orientaci napříč funkcionalitami v~pozdějším vývoji aplikace. Aktuální verze routeru má vždy na počátku cesty prefix "/api/v1", který nebude v~názvu rout uváděn.
...
\begin{description}
\item[GET /me] vrátí informace o~přihlášeném uživateli
\item[GET /me/all] vrátí kompletního uživatele včetně referencí
\item[PUT /me] update uživatele
\item[GET /me/book] vrátí knihy, které uživatel vlastní (jedná se o~knihy vytvořené uživatelem)
\item[PUT /me/book/:id] update knihy, jimž je uživatel vlastníkem a~která má konkrétní ID
\item[GET /me/project] vrátí projekty, jimiž je uživatel vlastníkem
\item[PUT /me/project/:id] update projektu, jimž je uživatel vlastníkem a~které mají konkrétní ID
\item[GET /me/project/:id/render] vrátí render konkrétního projektu
\item[POST /me/project] vytvoří nový projekt
\item[PUT /me/project] aktualizuje projekt
\item[GET /me/order/:id] vrátí objednávku
\item[POST /me/order] vytvoří novou objednávku
\item[PUT /me/order] update objednávky
\end{description}

\subsubsection{/book}
API pro knihy slouží pro práci s~knihami, z~uživatelského hlediska jde především o~hledání a~výpisy.
\begin{description}
\item[GET /book] vrátí pole knih na základě query stringu
\item[GET /book/:id] vrátí kompletní informace o~knize
\item[GET /book/:id/all] vrátí kompletní informace o~knize včetně populate
\item[PUT /book/:id] update knihy
\item[POST /book] vytvoření knihy
\item[DELETE /book/:id] smaže knihu
\end{description}

\subsubsection{/user}
Tato sada rout slouží pro práci s~uživateli a~je třeba být přihlášen jako administrátor pro přístup
\begin{description}
\item[GET /user] vrátí pole uživatelů na základě query stringu
\item[GET /user/:id] vrátí kompletní informace o~uživateli včetně referencí
\item[PUT /user/:id] update uživatele
\item[POST /user] vytvoření uživatele (nejedná se o~registraci)
\item[DELETE /user/:id] smaže uživatele
\end{description}

\subsubsection{/branch}
Routy slouží pouze pro práci s~pobočkami a~uživatelé nemají k~těmto informacím přístup možné je pouze vypsat pobočky.
\begin{description}
\item[GET /branch] vrátí pole poboček na základě query stringu
\item[GET /branch/:id] vrátí kompletní informace o~pobočce
\item[PUT /branch/:id] update pobočky
\item[POST /branch] vytvoření pobočky
\item[PUT /branch] vytvoření pobočky
\item[POST /branch/employe] vytvoření zaměstnance
\item[PUT /branch/employe] update zaměstnance
\item[POST /branch/order] vytvoření objednávky
\item[PUT /branch/order] update objednávky
\item[DELETE /branch/:id] smaže branch
\end{description}

\subsubsection{/order}
Routy slouží pouze pro práci s~objednávkami a~jsou přístupné pouze oprávněným uživatelům.
\begin{description}
\item[GET /order] vrátí pole objednávek na základě query stringu
\item[GET /order/:id] vrátí kompletní informace o~objednávce včetně referencí
\item[PUT /order/:id] update objednávky
\item[POST /order] vytvoření objednávky
\item[DELETE /order/:id] smaže objednávku
\end{description}
...
eyJ0eX...JIUzI1NiJ9.eyJ1c...DU2Nzg5fQ.KG...1yB2xAwc_E
    \end{verbatim}
\end{codeframe}
header a~payload jsou pouze jednoduché JSON objekty, které vypadají jako v~následující ukázce:\cite{JWTTut}
\b{header:}
\begin{codeframe}
  \begin{verbatim}
{ "typ": "JWT", "alg": "HMACSHA256" }
    \end{verbatim}
\end{codeframe}
typ je vždy JWT a~alg je hash algoritmus použitý pro podepsání tokenu.\cite{JWTTut}

\b{payload:} může býť jakýkoliv validní JSON objekt, ale vyžaduje některé standardizované klíče.
\begin{codeframe}
  \begin{verbatim}
{ "user": "Franta", "exp": 123456789}
    \end{verbatim}
\end{codeframe}
user je klíč definovaný a~exp je standardní UNIX time s~časem do kdy token platí. To že payload může být libovolný je skvělá věc, protože~můžeme přenášet jakékoliv informace o~uživateli. Důležité je vědět, že~payload není šifrovaný, jedná se pouze o~base64 zakódování, a nemůže být tedy použit k~přenášení citlivých informací.\\
...
\subsubsection{Implementace backend (NODEJS)}
Implementace na straně serveru řeší doplněk \it{hapi-auth-jwt}, kterému stačí zadat master klíč aplikace a~validační funkci tokenu, která dostává dekódovaný payload tokenu a~může tak ověřit informace o~uživateli. Token je pak možné vygenerovat pomocí jednoduché funkce "jwt.sign":
\begin{codeframe}
  \begin{verbatim}
token = jwt.sign(
    name: this.name
  ...
  ,
    ...
    \end{verbatim}
\end{codeframe}
Stejně jako bylo uvedeno v~předchozí kapitole, do části tokenu zvané payload je možné uložit jakékoliv informace, záleží jenom na programátorovi jak velké množství dat je únosné. Protože token je přenášen v~každém requestu, není vhodné to s~přenášenými daty nijak přehánět. Token použitý v~mojí aplikaci, přenáší základní data o~uživateli, včetně jeho práv.
...
\subsection{MongoDB}
V~rozdělení zátěže a~lepšího škálování služeb přispívá i~samotné MongoDB, které umožňuje sharding několika MongoDB serverů a~replikaci data. Sharding, je ale velmi náročný a~k~zprovoznění shardingu je třeba alespoň 7mi serverů, 3~konfigurační, 2~a~více routerů a~2~a~více replik.\cite{sharding}
\begin{figure}[h]
\begin{center}$
\begin{array}{c}
...
Škálovatelnost databáze samozřejmě řešíme pouze pokud využíváme vlastní infrastruktury nebo některé cloudové IaaS\footnote{Infrastructure as service - cloudová služba kdy uživatel získá jako by přímo vlastní diskový a~výpočetní výkon a~chová se k~němu jako vlastnímu dedikovanému serveru} služby jako je Amozon EC2. V~případě, že využíváme pro databázi některou ze služeb SaaS\footnote{Service as Service - uživateli je pronajímána pouze služba s~omezeným diskovým prostorem a~výpočetním výkonem} neřešíme škálovatelnost, záleží totiž na tom jaké podpůrné služby si zaplatíme. Takže v~jedné ceně můžeme mít replikovaný cluster včetně záloh.

\subsubsection{Cloudové služby}
Mezi známější služby nabízející MongoDB patří např. \href{http://mongodirector.com/}{mongodirector.com} nebo \href{http://mongolab.com}{mongolab.com} obě služby jsou podobné, nabízejí MongoDB service v~IaaS službách jiných poskytovatelů např. Amazon AWS, Windows Azure apd,… Infrastruktura je dle zvoleného plánu během několika minut aktivní a~použitelná, bez jakékoliv znalosti nastavení serverů a~řešení jejich zabezpečení. Přičemž ceny vhodné pro provoz poměrně velkých webů začínají kolem 100\$, kdy je možné získat dva replikované mongoDB servery 2GB RAM, 15GB SSD, dvě procesorové jádra a~možnost kdykoliv zvýšit výkon pouhým kliknutím.
...
\bibitem{ES6browserify}http://corner.squareup.com/2013/02/es6-module-transpiler.html
\bibitem{ES6browserify2}http://www.infoq.com/news/2013/08/es6-modules
\bibitem{browserify} http://browserify.org/
\bibitem{browserify2} http://esa-matti.suuronen.org/blog/2013/03/22/journey-from-requirejs-to-browserify/
\bibitem{bootstrap} http://www.zdrojak.cz/clanky/k-cemu-je-dobry-bootstrap-frontend-frameworky/
\bibitem{bootstrap2} http://bootstrap.lesscss.ru/less.html
\bibitem{bootstrap3}http://getbootstrap.com/getting-started/
\bibitem{angularHTTP} https://docs.angularjs.org/api/ng/service/\$http
\bibitem{lesshat} https://github.com/madebysource/lesshat/blob/master/README.md
\bibitem{lesshat2} http://designshack.net/articles/css/battle-of-the-less-mixin-libraries-less-elements-vs-less-hat-vs-bootstrap/
...
\bibitem{jsonZran} http://haacked.com/archive/2008/11/20/anatomy-of-a-subtle-json-vulnerability.aspx/
\bibitem{jsonZran2} http://jeremiahgrossman.blogspot.cz/2006/01/advanced-web-attack-techniques-using.html
\bibitem{mongoAdmin} http://docs.mongodb.org/manual/tutorial/add-user-administrator/
\bibitem{jakvydatknihu} http://jakvydatknihu.wikina.cz/
\bibitem{lupainternet} http://www.lupa.cz/clanky/vydejte-si-vlastni-knihu-pres-internet/
...
\bibitem{JWT} http://self-issued.info/docs/draft-ietf-oauth-json-web-token.html
\bibitem{JWTTut} http://mikehadlow.blogspot.cz/2014/04/json-web-tokens-owin-and-angularjs.html
